...
if(isset($_POST["sendBulkmail"])){
    ...
    if(!empty($mailList) &&  trim($mailContent) != ""   &&  trim($subject) !=""){
		foreach ($mailList as $key => $to) {
			...
			$mail->FromName = "Admin";
			$mail->addAddress("anishpraba@hotmail.com", "Admin");
			$mail->isHTML(true);
		...
		echo "<br>Error. Please check email sender list empty or mail content or mail subject";
	}
}
...
<form name="bulkmail" method="post" class="pure-form pure-form-stacked">
	...
	<p style="padding-left:50px;">Subject: <input type="text" name="subject" size="50" /></p>
	<p style="padding-left:50px;">Mail Content: <br><textarea rows="3" cols="80" name="mailContent" ></textarea></p>
